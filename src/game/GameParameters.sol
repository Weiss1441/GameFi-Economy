...
contract GameParameters is Ownable, IGameParameters {
    ...
    function getDropRate(uint256 itemId) 
        external 
        view 
        override 
        returns (uint256) 
    ...
    function getCraftingCost(uint256 recipeId) 
        external 
        view 
        override 
        returns (uint256) 
    ...
    function setDropRate(uint256 itemId, uint256 rate) 
        external 
        override 
        onlyOwner 
    {
    ...
    function setMultipleDropRates(uint256[] calldata itemIds, uint256[] calldata rates) 
        external 
        onlyOwner 
    {
