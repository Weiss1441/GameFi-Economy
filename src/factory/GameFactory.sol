...
contract GameFactory {
    ...
    function deployGameCREATE(string memory name, string memory symbol) external returns (address, address) {
        ...
        deployments.push(GameDeployment({
            gameItems: address(items),
            gameParams: address(params),
            deployer: msg.sender,
        ...
        returns (address params, address items)
    {
        ...
        bytes32 paramsHash = keccak256(abi.encodePacked(
            ...
            address(this),
            saltBytes,
            keccak256(paramsCreationCode)
        ));
        params = address(uint160(uint256(paramsHash)));
        
        bytes memory itemsCreationCode = abi.encodePacked(
            type(GameItems).creationCode,
            abi.encode(name, symbol, params)
        );
        bytes32 itemsHash = keccak256(abi.encodePacked(
            bytes1(0xff),
            address(this),
            saltBytes,
            keccak256(itemsCreationCode)
        ));
